...
string receive_message(int sock) {
 ...
 void parse_online_list(const string& response) {
    ...
    // First line: account balance (may be preceded by status messages like "Transfer OK!")
    // Skip non-numeric lines until we find the balance
    ...
    while (getline(iss, line)) {
        ...
        // Check if line contains a valid number (balance)
        try {
            if (!line.empty() && (isdigit(line[0]) || line[0] == '-')) {
                account_balance = stoi(line);
                cout << "Account Balance: $" << account_balance << endl;
                balance_found = true;
                break;  // Found balance, continue to next field
            }
            // If not a number, it might be a status message, skip it
        } catch (const std::exception& e) {
            // Not a number, continue to next line
            continue;
        }
    }
    
    if (!balance_found) {
        cout << "[ERROR] Could not find balance in server response." << endl;
        return;
    }
     ...
     if (pos1 != string::npos && pos2 != string::npos) {
        ...
        safe_print("[DEBUG] Parsed P2P: sender='" + sender + "', amount='" + amount_str + "', recipient='" + recipient + "'");
         ...
         if (is_logged_in && server_socket != -1) {
            ...
            // Send transaction report to server: sender#amount#recipient
            // According to spec: recipient should encrypt the message with server's public key
            // 根據規格：收款方應使用 Server 的 public key 加密訊息後發送
            // Format: sender#amount#recipient (should be encrypted with server_public_key)
            // 格式：sender#amount#recipient（應使用 server_public_key 加密）
            string transaction_msg = sender + "#" + amount_str + "#" + recipient;
             
             safe_print("[DEBUG] Reporting to server: '" + transaction_msg + "'");
             
             if (send_message(server_socket, transaction_msg)) {
                 // Server may not respond immediately to transaction reports
                 // Use non-blocking check or timeout to avoid blocking
                 // Server 可能不會立即回應交易報告，使用非阻塞檢查避免阻塞
                 
                 // Try to receive response with a short timeout simulation
                 ...
                 if (select_result > 0 && FD_ISSET(server_socket, &readfds)) {
                     string response = receive_message(server_socket);
                     if (!response.empty()) {
                         safe_print("[DEBUG] Server response: '" + response + "'");
                     }
                 } else {
                     safe_print("[DEBUG] Server did not respond immediately (this is normal)");
                 }
